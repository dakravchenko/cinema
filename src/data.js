...
export async function getFilmsByName(query) {
  const url = `https://api.themoviedb.org/3/search/movie?api_key=48dff5b67e789d262126f79823385995&language=en-US&query=${query}&page=1&include_adult=false`;
  try {
...
export async function getGenres() {
  try {
    const res = await fetch(
      "https://api.themoviedb.org/3/genre/movie/list?api_key=48dff5b67e789d262126f79823385995&language=en-US"
    );
    if (!res.ok) {
      throw new Error("HTTP error");
    }
...
export async function getTopMovies(page) {
  try {
    const res = await fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=48dff5b67e789d262126f79823385995&language=en-US&page=${page}`
    );
    if (!res.ok) {
      throw new Error("HTTP error");
    }
