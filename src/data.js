...
export async function getGenres() {
  try {
    const res = await fetch(
      "https://api.themoviedb.org/3/genre/movie/list?api_key=464e0732a4ff7c3b5e09de7baa51e9f2&language=en-US"
    );
    if (!res.ok) {
      throw new Error("HTTP error");
    }
...
export async function getTopMovies(page) {
  try {
    const res = await fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=464e0732a4ff7c3b5e09de7baa51e9f2&language=en-US&page=${page}`
    );
    if (!res.ok) {
      throw new Error("HTTP error");
    }
